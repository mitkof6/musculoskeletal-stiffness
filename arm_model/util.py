...
def convex_bounded_vertex_enumeration(A,
    ...
    if method == 'lrs':
        ...
        raise RuntimeError(
            'Unsupported method: choose "lrs" or "cdd" or "rnd"')
...
def draw_ellipse(ax, xc, A, scale=1.0, show_axis=True):
...
def calculate_stiffness_properties(stiffness_analysis, base_name,
                                   t_start, t_end, dt, speed):
    """ Calculates the stiffness properties of a simulation.

    Parameters
    ----------

    stiffness_analysis: StiffnessReporter
    ...
    """
    print('Calculating feasible stiffness properties ...')
    N = 3
    time = np.linspace(0.0, t_end, N, endpoint=True)
    alpha = np.linspace(0.1, 1.0, N, endpoint=True)
    scaling = 0.0006
    # collective results
    fig = plt.figure(figsize=(19, 6))
    ax = np.array([fig.add_subplot(1, 3, 1),
                   fig.add_subplot(1, 3, 2),  # projection='3d'),
                   fig.add_subplot(1, 3, 3)])
    stiffness_analysis.dataframe = pd.DataFrame()
    axis_limits = [[0, 0.07], [0.97, 1], [0, 180]]
    # axis_limits = None
    for i, t in enumerate(tqdm(time)):
        # try:
        stiffness_analysis.visualize_stiffness_properties(round(t, 1),
                                                          True,
                                                          scaling,
                                                          alpha[i], ax,
                                                          axis_limits)
        # except:
        #     print('Infeasible Inequality at t = ' + str(t))
        #     continue

    fig.tight_layout()
    fig_name = base_name + 'collection'
    fig.savefig(fig_name + '.png', format='png', dpi=300)
    ...
    for i, t in enumerate(tqdm(time)):
        ...
        fig.tight_layout()
        fig_name = base_name + str(i).zfill(6)
        fig.savefig(fig_name + '.png', format='png', dpi=300)
        fig.savefig(fig_name + '.pdf', format='pdf', dpi=300)
        fig.savefig(fig_name + '.eps', format='eps', dpi=300)
